...
const ThemeToggle: React.FC<ThemeToggleProps> = ({ compact = false, className = '' }) => {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`touch-target motion-lift inline-flex items-center gap-2 rounded-xl border border-secondary-700 bg-secondary-900 px-2.5 py-2 text-secondary-200 hover:bg-secondary-800 ${className}`}
